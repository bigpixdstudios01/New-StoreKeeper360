...
api.post("/products", authMiddleware, async (req, res) => {
  const { name, sku, category_id, purchase_price, selling_price, quantity, restock_level, unit, description } = req.body;
...
api.put("/products/:id", authMiddleware, async (req, res) => {
  const { name, sku, category_id, purchase_price, selling_price, restock_level, unit, description } = req.body;
  ...
  const { error } = await supabaseAdmin
    .from("products")
    .update({
      name, sku: sku || null, category_id: category_id || null,
...
api.post("/sales/bulk", authMiddleware, async (req, res) => {
  const { items, note, sale_date, payment_method, bank_account_id, pos_terminal_id } = req.body;
  ...
  for (const { item, product } of productChecks) {
    ...
    const revenue = price * item.quantity_sold;
...
api.get("/sales", authMiddleware, async (req, res) => {
  ...
  const baseSelect = "*, products(name, unit, categories(name)), bank_accounts(bank_name, account_number, account_name), pos_terminals(terminal_name, provider)";
